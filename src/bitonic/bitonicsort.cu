...
)
{
    ...
    if (arrayLength <= SHARED_SIZE_LIMIT)
    {
        ...
        bitonicSortShared<<<blockCount, threadCount, 0, stream>>>(d_DstKey, d_DstVal, d_SrcKey, d_SrcVal, arrayLength, dir);
    }
    else
    {
        bitonicSortShared1<<<blockCount, threadCount, 0, stream>>>(d_DstKey, d_DstVal, d_SrcKey, d_SrcVal);
        cudaError_t errSync = cudaGetLastError();
		//cudaError_t errAsync = cudaStreamSynchronize(stream);
        ...
        for (uint size = 2 * SHARED_SIZE_LIMIT; size <= arrayLength; size <<= 1)
            for (unsigned stride = size / 2; stride > 0; stride >>= 1)
                ...
                {
                    bitonicMergeGlobal<<<(batchSize * arrayLength) / 512, 256, 0, stream>>>(d_DstKey, d_DstVal, d_DstKey, d_DstVal, arrayLength, size, stride, dir);
                }
                else
                {
                    bitonicMergeShared<<<blockCount, threadCount, 0, stream>>>(d_DstKey, d_DstVal, d_DstKey, d_DstVal, arrayLength, size, dir);
                    break;
                }
    }
